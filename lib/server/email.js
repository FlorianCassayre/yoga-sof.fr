...
const dispatchEmailFromComponent = async (user, emailType, subject, component) => await dispatchEmail(user, emailType, subject, renderToStaticMarkup(component));
...
export const notifySessionCanceled = async session => {
  ...
  await Promise.all(
    registrations
      ...
      .filter(({ user: { email, receive_emails } }) => email && receive_emails)
      .map(({ user }) => dispatchEmailFromComponent(user, EMAIL_TYPE_SESSION_CANCELED, emailSessionCanceledSubjectFor(), <EmailSessionCanceledBody user={user} session={session} />)),
  );
};
