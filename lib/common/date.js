...
export const formatTime = (time, compact = false) => {
  // Input is HH:mm
...
const dateLiteralFormatter = new Intl.DateTimeFormat(locale, {
  dateStyle: 'full',
});
...
export const formatTimeRange = (start, end) => [timeMinutesFormatter.format(new Date(start)).replace(':', 'h'), 'à', timeMinutesFormatter.format(new Date(end)).replace(':', 'h')].join(' ');

const capitalizeFirst = s => (s.length ? s[0].toUpperCase() + s.slice(1) : s);

export const formatDateLiteral = date => capitalizeFirst(dateLiteralFormatter.format(new Date(date)));

export const formatDayRange = (start, end, capitalize = true) => {
...
export const dateToParsedTime = date => {
