import { displaySessionName } from './display';
...
function EmailContent({ user: { name }, children }) {
  return (
    <>
      {name}
      ,
...
export function EmailSessionCanceledBody({ user, session }) {
  return (
    <EmailContent user={user}>
      La
      {' '}
      {displaySessionName(session, false)}
      {' '}
